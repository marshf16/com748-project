...
def load_data(data_dir):
    # Using training data folder, add images and their label to a list suitable for the CNN model
...
def train_models(data_dir, IMG_HEIGHT=30, IMG_WIDTH=30, EPOCHS=30, save_model = True, saved_model = "sign_detection_model.h5"):
    train_path = data_dir + '/test_images'
    ...
    # Fitting the model
    history = model.fit(x_train, y_train, validation_data=(x_test, y_test), epochs=EPOCHS, steps_per_epoch=34)
...
def evaluate_model(model_path, image_path = "", image_label = None):
    model = load_model(model_path)
    model.summary()

    # load dataset
    if image_path == "":
        def_test_images_path = "src/self_driving_car/data/test_images"
...
def main():
    train_models(os.path.abspath("src/self_driving_car/data/"))
    evaluate_model(os.path.abspath("src/self_driving_car/data/sign_detection_model.h5"))
